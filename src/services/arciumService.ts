...
export const ARCIUM_CONFIG = {
  ...
  ARCIUM_PROGRAM_ID: new PublicKey('ArcmKNYRXmCkr6R3qXgGBiSYoZakZNrVCWA1o7pUSVc'),
  
  // MnM Privacy MXE program - deployed to Arcium network
  // This handles encrypted position state and private health checks
  MNM_PRIVACY_MXE_ID: new PublicKey('MNMpRiv8qYZ9kMnGzVNP1Y5bvBLXZ2aGexDfFsWTRqz'),
  
  // Default cluster for MPC computations
  DEFAULT_CLUSTER: new PublicKey('CLSTRk8x9KPzEg6CxJJnhGvGAcLkG7R9NKRbXkxEL4LX'),
...
export async function privateHealthCheck(
  ...
  provider?: AnchorProvider
): Promise<{ isLiquidatable: boolean; proofOfComputation: Uint8Array; computationId: string }> {
  ...
  console.log(`[Arcium] Threshold: ${liquidationThreshold}, Position owner: ${encryptedPosition.owner.toBase58().slice(0, 8)}...`);
  ...
  if (provider) {
    try {
      ...
      if (mxePublicKey) {
        console.log(`[Arcium] MXE public key fetched, submitting to MPC cluster...`);
      ...
      console.warn('[Arcium] MPC call failed, falling back to simulation:', error);
    }
  }
  ...
  console.log(`[Arcium] Health check complete: isLiquidatable=${isLiquidatable}`);
  ...
  async initialize(provider: AnchorProvider): Promise<boolean> {
    try {
      ...
      if (this.mxePublicKey) {
        console.log('[ArciumPrivacyService] Connected to MXE, public key:', 
          Buffer.from(this.mxePublicKey).toString('hex').slice(0, 16) + '...');
        this.initialized = true;
      } else {
        console.log('[ArciumPrivacyService] MXE not available, running in simulation mode');
