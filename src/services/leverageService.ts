...
const JUPITER_FLASH_LOAN_PROGRAM = new PublicKey('JUP6LkbZbjS1jKKwapdHNy74zcZ3tLUZoi5QNyVTaV4');
...
export function validateLeverageParams(
  ...
  if (targetLeverage > maxLeverage) {
    return { valid: false, error: `Maximum leverage is ${maxLeverage.toFixed(1)}x` };
  }
  ...
  if (baseAmount < MIN_POSITION_USD) {
    return { valid: false, error: `Minimum position size is $${MIN_POSITION_USD}` };
  }
...
export async function buildLeverageTransaction(
  ...
  const tokenXAmountBN = new BN(Math.floor(amounts.tokenXAmount * Math.pow(10, tokenXDecimals)));
  const tokenYAmountBN = new BN(Math.floor(amounts.tokenYAmount * Math.pow(10, tokenYDecimals)));
  ...
  const liquidationThreshold = amounts.borrowRequired / RISK_PARAMS.LIQUIDATION_THRESHOLD;
...
export async function getUserLeveragedPositions(
  ...
  const collateralPositions = await collateralService.getUserCollateralPositions(
    connection,
    user,
    prices
  );
