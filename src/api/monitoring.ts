...
export interface MonitoringAlert {
  type: 'out_of_range' | 'yield_drop' | 'yield_spike' | 'rebalance_needed' | 'fee_harvest';
...
export function checkPositionHealth(position: {
  ...
  // Check if in range
  const inRange = position.currentPrice >= position.rangeMin && 
  ...
  // Out of range alert
  if (!inRange) {
    const direction = position.currentPrice < position.rangeMin ? 'below' : 'above';
    alerts.push({
      type: 'out_of_range',
      severity: 'urgent',
  ...
  if (inRange && (utilizationPercent < 10 || utilizationPercent > 90)) {
    alerts.push({
      ...
      suggestion: 'Consider widening range or rebalancing soon to avoid going out of range',
...
export function checkPoolHealth(pool: {
  ...
  if (apyChange24h > 50) {
    alerts.push({
      ...
      data: { currentAPY: pool.currentAPY, previousAPY: apy24hAgo, change: apyChange24h },
      timestamp: now,
    });
  }
...
export function formatHealthReport(positions: PositionHealth[]): string {
  ...
  const urgentAlerts = positions.flatMap(p => p.alerts.filter(a => a.severity === 'urgent'));
  const warnings = positions.flatMap(p => p.alerts.filter(a => a.severity === 'warning'));
...
export function formatPoolReport(pools: PoolHealthCheck[]): string {
  ...
  const opportunities = pools.filter(p => p.alerts.some(a => a.type === 'yield_spike'));
  const warnings = pools.filter(p => p.alerts.some(a => a.severity === 'warning'));
