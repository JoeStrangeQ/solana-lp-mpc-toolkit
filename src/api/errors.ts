...
export const ERRORS = {
  // Connection errors
  API_UNREACHABLE: (endpoint: string) =>
    createAgentError(
      "API_UNREACHABLE",
  ...
  RPC_ERROR: (rpc: string, error: string) =>
    createAgentError(
      "RPC_ERROR",
  ...
  INVALID_TOKEN: (token: string) =>
    createAgentError(
      ...
      {
        token,
        supportedTokens: ["SOL", "USDC", "USDT", "BONK", "JTO", "JUP", "RAY"],
      },
    ),

  INVALID_AMOUNT: (amount: any) =>
    createAgentError(
      "INVALID_AMOUNT",
  ...
  // Pool errors
  NO_POOLS_FOUND: (tokenA: string, tokenB: string) =>
    createAgentError(
      "NO_POOLS_FOUND",
  ...
  INSUFFICIENT_LIQUIDITY: (pool: string, required: number, available: number) =>
    createAgentError(
  ...
  POSITION_NOT_FOUND: (positionId: string) =>
    createAgentError(
      "POSITION_NOT_FOUND",
  ...
  SLIPPAGE_EXCEEDED: (expected: number, actual: number, maxSlippage: number) =>
    createAgentError(
      "SLIPPAGE_EXCEEDED",
  ...
  TX_SIMULATION_FAILED: (error: string) =>
    createAgentError(
  ...
  // Encryption errors
  ENCRYPTION_FAILED: (reason: string) =>
    createAgentError(
      "ENCRYPTION_FAILED",
