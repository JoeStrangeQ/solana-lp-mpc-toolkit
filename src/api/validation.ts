 ...
 */
export function validateAmount(input: any, options?: {
  min?: number;
  max?: number;
  allowZero?: boolean;
}): ValidationResult {
...
const VALID_TOKENS = ['SOL', 'USDC', 'USDT', 'BONK', 'JTO', 'JUP', 'RAY', 'MSOL', 'STSOL', 'WIF', 'PYTH'];
...
export function validateTokenSymbol(input: any): ValidationResult {
  ...
  return { 
    valid: true, 
    sanitized,
    ...(isKnown ? {} : { error: `Warning: ${sanitized} is not a recognized token` })
  };
}

// ============ Venue Validation ============

const VALID_VENUES = ['meteora', 'meteora-damm', 'orca', 'raydium', 'lifinity', 'saber', 'crema', 'fluxbeam', 'invariant'];
...
export function validateSlippage(input: any): ValidationResult {
  ...
  if (!result.valid) {
    return { valid: false, error: 'Slippage must be 1-5000 basis points (0.01% - 50%)' };
  }
...
const VALID_STRATEGIES = ['balanced', 'concentrated', 'yield-max', 'delta-neutral', 'bid-heavy', 'ask-heavy'];
...
export function validateAddLiquidityRequest(body: any): ValidationResult {
  ...
  if (body.amountA !== undefined) {
    const amountAResult = validateAmount(body.amountA, { min: 0, allowZero: true });
  ...
  if (body.amountB !== undefined) {
    const amountBResult = validateAmount(body.amountB, { min: 0, allowZero: true });
