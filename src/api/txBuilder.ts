...
export async function buildAddLiquidityTx(
  ...
  try {
    const { userPubkey, poolAddress, venue, tokenA, tokenB, amountA, amountB } = params;
...
export async function buildRemoveLiquidityTx(
  ...
  try {
    ...
    return {
      ...
      instructions: [`Remove ${percentage}% from ${venue} position ${positionId.slice(0, 8)}...`],
    };
