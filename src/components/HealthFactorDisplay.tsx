    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className={`${currentSize.icon} rounded-full ${currentColors.bg} ${
        ...
        <div className="flex items-center gap-2">
          <span className={`${currentSize.text} font-mono font-bold ${currentColors.text}`}>
            {healthFactor.toFixed(2)}
          </span>
          
          <span className={`text-xs px-2 py-0.5 rounded ${currentColors.bg}/20 ${currentColors.text}`}>
            {statusLabels[status]}
          </span>
        </div>
      </div>
      ...
      {showBar && (
        <div className="relative">
          {/* Background bar */}
          <div className={`w-full ${currentSize.bar} bg-gray-700 rounded-full overflow-hidden`}>
            {/* Fill bar */}
            <div
      ...
      {status === 'danger' && (
        <div className="flex items-center gap-2 text-orange-400 text-sm bg-orange-500/10 rounded-lg p-2">
          ...
          <span>Position at risk! Consider adding collateral or reducing debt.</span>
        </div>
      )}
