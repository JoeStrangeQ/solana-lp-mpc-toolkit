...
import { Address, getMarketFromMints, mints, tokensMetadata } from "../../convex/utils/solana";
...
const swapToItems: { id: SwapToOption; element: ReactNode }[] = [
  {
    id: "none",
    element: <div className={cn("text-xs font-normal whitespace-nowrap", "text-text")}>No Swap</div>,
  },
  {
    id: "SOL",
    element: (
      <Row justify="start" className={cn("gap-1 text-xs font-normal whitespace-nowrap", "text-text")}>
        <TokenIcon className="w-3.5 h-3.5" icon={tokensMetadata[mints.sol].icon} />
        SOL
      </Row>
    ),
  },
  {
    id: "USDC",
    element: (
      <Row justify="start" className={cn("gap-1 text-xs font-normal whitespace-nowrap", "text-text l")}>
        <TokenIcon className="w-3.5 h-3.5" icon={tokensMetadata[mints.usdc].icon} />
        USDC
      </Row>
    ),
  },
...
export function LimitOrderValues({
  ...
  return (
    <>
      <Row
        justify="start"
        className={cn(" gap-px ", !disableEdit && " group cursor-pointer active:scale-95")}
      ...
      >
        ...
        <div className="text-sm text-text group-hover:text-text font-normal hover-effect group-hover:underline">/</div>
...
export function LimitOrdersModalContent({
  ...
  const [sl, setSl] = useState<LimitOrderInputType>(initialSl ?? { price: 0, swapTo: "none" });

  const [tp, setTp] = useState<LimitOrderInputType>(initialTp ?? { price: 0, swapTo: "none" });

  const invalidSl = sl.price !== undefined && sl.price !== 0 && sl.price >= poolPrice;
  const invalidTp = tp.price !== undefined && tp.price !== 0 && tp.price <= poolPrice;

  const isInitialSl = initialSl ? sl.price === initialSl.price && sl.swapTo === initialSl.swapTo : sl.price === 0; // unchanged when no initial was given
  const isInitialTp = initialTp ? tp.price === initialTp.price && tp.swapTo === initialTp.swapTo : tp.price === 0;
  ...
  return (
    <div className="flex flex-col w-[440px]">
      ...
      <div className="flex flex-col gap-1 mb-4">
        <LabelValue variant="row" label={"Market"} value={market} labelClassName="text-xs" />
        <LabelValue
          variant="row"
          label={"Pool Price"}
        ...
        />
        <LabelValue variant="row" label={"Oracle Price"} value={0} labelClassName="text-xs" />
      </div>
      <LimitOrderInput
        direction="sl"
        order={sl}
      ...
      <Button variant="liquidPrimary" onClick={() => onSaveOrders(sl, tp)} disabled={disableSave}>
        Save
      </Button>
    </div>
  );
...
function LimitOrderInput({
  ...
  useEffect(() => {
    if (order.price === 0) {
      ...
      const clamped = pct < 0 ? "0" : direction === "sl" && pct > 100 ? "100" : pct.toFixed(2).replace(/\.00$/, "");
      ...
      <Row fullWidth justify="start">
        ...
        >
          <input
            ...
            onChange={(e) => {
              ...
              // Validation
              if (direction === "sl" && num >= marketPrice) return setErr("Stop loss must be below market price");

              if (direction === "tp" && num <= marketPrice) return setErr("Take profit must be above market price");
        ...
        >
          ...
          <div className="text-textSecondary text-xs mr-1">
            {!order.price && direction === "tp" ? "+" : (order.price ?? 0) >= marketPrice ? "+" : "-"}
          </div>
...
function onPercentageChange({
  ...
  const newPrice = direction === "sl" ? marketPrice * (1 - num / 100) : marketPrice * (1 + num / 100);

  setPriceInput(String(newPrice));
  onPrice(newPrice);

  // Validation
  if (direction === "sl" && newPrice >= marketPrice) return setErr("Stop loss must be below market price");

  if (direction === "tp" && newPrice <= marketPrice) return setErr("Take profit must be above market price");
