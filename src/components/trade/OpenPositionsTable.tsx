  ...
  return (
    <div className="w-full overflow-x-auto rounded-2xl bg-backgroundSecondary p-4 custom-scrollbar">
      <Table className="table-auto">
        ...
        {convexUser ? (
          <OpenPositions convexUser={convexUser} />
        ) : (
          <NoPositionPlaceholder />
        )}
      </Table>
    </div>
  );
...
function OpenPositions({ convexUser }: { convexUser: Doc<"users"> }) {
  const openDbPositions = useQuery(
    api.tables.positions.get.getUserOpenPositions,
