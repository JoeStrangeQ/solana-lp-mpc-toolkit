...
export class YieldMonitor {
  ...
  async checkAndReport(): Promise<YieldUpdate[]> {
    ...
    const aggregated = await this.scanner.getAggregatedPositions(this.ownerPubkey);
    ...
    for (const pos of positions) {
      ...
      const feesDelta = pos.unclaimedFees.totalUSD - (lastPos.unclaimedFees?.totalUSD || 0);
      if (pos.unclaimedFees.totalUSD >= this.config.alertThresholds.feesReadyUSD && feesDelta > 0) {
        updates.push({
          timestamp: now,
          type: 'alert',
          priority: 'low',
      ...
      const valueChange = ((pos.valueUSD - lastPos.valueUSD) / lastPos.valueUSD) * 100;
      if (Math.abs(valueChange) >= this.config.alertThresholds.valueChangePercent) {
