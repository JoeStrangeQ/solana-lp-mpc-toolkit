...
// Token patterns
const TOKEN_PATTERN = /\b(SOL|USDC|USDT|mSOL|stSOL|JTO|JUP|BONK|RAY|WIF|PYTH|ETH|BTC)\b/gi;
...
export function parseIntent(text: string): ParsedIntent {
  ...
  if (amountMatch) {
    ...
    params.amountToken = suffix === 'sol' ? 'SOL' : 
                         suffix === 'usdc' ? 'USDC' : 'USD';
  }
  ...
  for (const pattern of ADD_PATTERNS) {
    if (pattern.test(normalized)) {
      return { type: 'add_liquidity', confidence: 0.85, params, originalText: text };
    }
  }
  ...
  for (const pattern of REMOVE_PATTERNS) {
    if (pattern.test(normalized)) {
      return { type: 'remove_liquidity', confidence: 0.85, params, originalText: text };
    }
  }
  ...
  for (const pattern of CLAIM_PATTERNS) {
    if (pattern.test(normalized)) {
      return { type: 'claim_fees', confidence: 0.85, params, originalText: text };
    }
  }
  ...
  for (const pattern of YIELD_PATTERNS) {
    if (pattern.test(normalized)) {
      return { type: 'check_yield', confidence: 0.8, params, originalText: text };
    }
  }
  ...
  for (const pattern of POSITION_PATTERNS) {
    if (pattern.test(normalized)) {
      return { type: 'show_positions', confidence: 0.85, params, originalText: text };
    }
  }
  ...
  for (const pattern of SCAN_PATTERNS) {
    if (pattern.test(normalized)) {
      return { type: 'scan_pools', confidence: 0.8, params, originalText: text };
    }
  }
...
export function isLPRelated(text: string): boolean {
  const lpKeywords = /\b(lp|liquidity|pool|defi|yield|apy|swap|dex|meteora|orca|raydium|position|stake|farm)\b/i;
