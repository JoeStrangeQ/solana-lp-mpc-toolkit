...
export async function handleQuote(
  ...
  if (!result.recommended) {
    throw new Error(
      `No pool found for ${params.tokenA}${params.tokenB ? `-${params.tokenB}` : ""}`,
    );
  }

  const pool = result.recommended;
  const strategy = params.strategy
    ? getStrategy(params.strategy as any)
    : STRATEGIES.balanced;
    ...
    switch (params.action) {
      case "add": {
        if (!params.poolAddress || !params.amountUSD) {
          return {
            success: false,
            error: "Missing poolAddress or amountUSD",
            fee: calculateFee(0),
          };
        }
      ...
      case "remove": {
        ...
        const fee = calculateFee(
          (amountUSD * (params.percentage || 100)) / 100,
        );
 ...
 */
export async function handleInterpret(text: string): Promise<{
  intent: ParsedIntent;
 ...
 */
export function handleRecommendStrategy(preferences: {
