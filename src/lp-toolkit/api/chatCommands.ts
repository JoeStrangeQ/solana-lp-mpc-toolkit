...
import { formatPoolsForChat, formatPositionsForChat, LPPool } from '../adapters/types';
 ...
 */
export async function handleScan(ctx: ChatContext, args: string[]): Promise<CommandResult> {
 ...
 * Handle /lp positions
 */
export async function handlePositions(ctx: ChatContext): Promise<CommandResult> {
  try {
 ...
 */
export async function handleAdd(ctx: ChatContext, args: string[]): Promise<CommandResult> {
  if (args.length < 2) {
    return {
      success: false,
 ...
 */
export async function handleRemove(ctx: ChatContext, args: string[]): Promise<CommandResult> {
  if (args.length < 1) {
    return {
      success: false,
  ...
  const aggregated = await ctx.scanner.getAggregatedPositions(ctx.userPubkey);
  const position = aggregated.positions.find(p => 
    p.positionId.startsWith(positionId) || p.positionId === positionId
  );
 ...
 */
export async function handleClaim(ctx: ChatContext, args: string[]): Promise<CommandResult> {
  if (args.length < 1) {
    return {
      success: false,
