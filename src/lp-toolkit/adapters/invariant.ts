...
const INVARIANT_PROGRAM = new PublicKey('HyaB3W9q6XdA5xwpU4XnSZV94htfmbmqJXZcEbRaJutt');
...
export class InvariantAdapter implements DEXAdapter {
  ...
  async getPools(connection: Connection): Promise<LPPool[]> {
    try {
      const response = await fetch('https://stats.invariant.app/pool_list/solana');
  ...
  async getPositions(connection: Connection, user: PublicKey): Promise<LPPosition[]> {
    try {
      const response = await fetch(`https://stats.invariant.app/positions/solana/${user.toBase58()}`);
      ...
      if (Array.isArray(data)) {
        return data.map((pos: any) => ({
          ...
          priceRange: pos.lowerTick !== undefined && pos.upperTick !== undefined ? {
    ...
    return [
      {
        ...
        fee: 0.30, tvl: 3500000, apy: 45.8, apy7d: 42.3, volume24h: 2000000,
      },
      {
        venue: 'invariant' as DEXVenue,
        ...
        fee: 0.01, tvl: 2000000, apy: 7.2, apy7d: 6.8, volume24h: 1200000,
      },
    ];
  }
}
