...
export const STRATEGIES: Record<LPStrategy, StrategyTemplate> = {
  balanced: {
    id: 'balanced',
    name: 'Balanced',
    description: 'Equal token split with moderate price range. Good starting point.',
    riskLevel: 'medium',
  ...
  concentrated: {
    id: 'concentrated',
    name: 'Concentrated',
    description: 'Tight range for maximum capital efficiency. Requires active management.',
    riskLevel: 'high',
    rangeWidthPercent: 5,
    rangeSkew: 0,
  ...
  'yield-max': {
    ...
    description: 'Wide range to capture more fees. Lower capital efficiency but more stable.',
    riskLevel: 'medium',
  ...
  'delta-neutral': {
    id: 'delta-neutral',
    name: 'Delta Neutral',
    description: 'Minimize directional exposure and impermanent loss. Lower yield but safer.',
    riskLevel: 'low',
  ...
  'bid-heavy': {
    ...
    description: 'More liquidity on buy side. Accumulate token A as price drops.',
    riskLevel: 'medium',
...
export function recommendStrategy(
  ...
  const { riskTolerance = 'medium', timeCommitment = 'passive', goal = 'yield', pairType = 'volatile' } = preferences;
...
export function needsRebalance(
  ...
  const priceChangePercent = Math.abs((currentPrice - rangeCenter) / rangeCenter) * 100;
