 ...
 * Uses Raydium SDK v2 for transaction building.
 */
...
export async function buildRaydiumAddLiquidity(
  ...
  const { execute } = await raydium.clmm.increasePositionFromBase({
    poolInfo,
    ownerPosition: positionLayout as unknown as Parameters<typeof raydium.clmm.increasePositionFromBase>[0]['ownerPosition'],
    ownerInfo: {
      useSOLBalance: true,
    },
    base: 'MintA',
  ...
  return {
    unsignedTransaction: Buffer.from(result.signedTx.serialize()).toString('base64'),
  };
}
