...
async function main() {
  ...
  await validate('Pool scan (SOL-USDC)', 'GET', '/v1/pools/scan?tokenA=SOL&tokenB=USDC&limit=5', {
    validate: (d) => d.success && typeof d.count === 'number',
  });

  // 6. Pool scan - single venue
  await validate('Pool scan (Meteora only)', 'GET', '/v1/pools/scan?venue=meteora&limit=3');
  ...
  // 12. Get positions
  await validate('Get positions', 'GET', '/v1/positions/11111111111111111111111111111111', {
    validate: (d) => d.success && typeof d.count === 'number',
  });

  // 13. Get positions - invalid wallet
  await validate('Get positions - invalid wallet', 'GET', '/v1/positions/invalid', {
  ...
  await validate('Build remove liquidity TX', 'POST', '/v1/tx/remove-liquidity', {
    body: {
      userPubkey: '11111111111111111111111111111111',
  ...
  await validate('Monitor positions', 'GET', '/v1/monitor/positions/11111111111111111111111111111111', {
    validate: (d) => d.success,
  });

  // 18. Monitor pools
  await validate('Monitor pools', 'GET', '/v1/monitor/pools?venue=meteora&limit=5', {
    validate: (d) => d.success,
  });
  ...
  const hasRateLimit = results.some(r => r.headers?.['X-RateLimit-Remaining']);
  console.log(`\nHeaders:`);
