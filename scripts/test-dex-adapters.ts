...
async function testAdapter(
  ...
  try {
    ...
    if (pools.length > 0) {
      ...
      console.log(`     Top pool: ${pools[0]?.name || pools[0]?.address || 'Unknown'}`);
    }
  } catch (error: any) {
...
async function main() {
  ...
  // Run all tests
  try { results.push(await testMeteora()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testMeteoraDamm()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testOrca()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testRaydium()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testLifinity()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testSaber()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testCrema()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testFluxBeam()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  try { results.push(await testInvariant()); } catch (e: any) { console.log(`  ❌ Fatal: ${e.message}`); }
  ...
  for (const r of results) {
    const status = r.status === 'PASS' ? '✅' : r.status === 'PARTIAL' ? '⚠️' : '❌';
  ...
  const workingAdapters = results.filter(r => r.poolsFound > 0).map(r => r.adapter);
  ...
  if (primaryWorking.length === 3) {
    ...
    console.log(`   ⚠️  Primary DEXs working: ${primaryWorking.join(', ') || 'None'}`);
  }
