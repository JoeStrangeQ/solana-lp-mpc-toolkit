...
async function main() {
  ...
  console.log(`   Client Private Key: ${Buffer.from(clientKeys.privateKey).toString('hex').slice(0, 32)}...`);
  console.log(`   Client Public Key:  ${Buffer.from(clientKeys.publicKey).toString('hex').slice(0, 32)}...`);
  
  const sharedSecret = deriveSharedSecret(clientKeys.privateKey, ARCIUM_DEVNET_CONFIG.mxePublicKey);
  ...
  console.log(`\n   Decrypted Intent:`);
  console.log(`     - Amounts: ${decryptedIntent?.amountA} / ${decryptedIntent?.amountB}`);
  ...
  const encryptedPosition = privacyService.encryptPositionValue(testValue, testFees);
  console.log(`   Encrypted (nonce: ${encryptedPosition.nonce.slice(0, 12)}...)`);
