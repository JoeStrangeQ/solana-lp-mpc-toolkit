...
async function main() {
  ...
  console.log(`   ${results[results.length-1].status === 'PASS' ? '✅' : '❌'} ${results[results.length-1].poolsFound} pools (${results[results.length-1].latencyMs}ms)`);
  ...
  results.push(await testAPI(
    ...
    (data) => ({
      ...
      sample: data.whirlpools?.[0]?.tokenA?.symbol + '-' + data.whirlpools?.[0]?.tokenB?.symbol || 'N/A',
    })
  ));
  console.log(`   ${results[results.length-1].status === 'PASS' ? '✅' : '❌'} ${results[results.length-1].poolsFound} pools (${results[results.length-1].latencyMs}ms)`);
  ...
  console.log(`   ${results[results.length-1].status === 'PASS' ? '✅' : '❌'} ${results[results.length-1].poolsFound} pools (${results[results.length-1].latencyMs}ms) ${results[results.length-1].error || ''}`);
  ...
  results.push(await testAPI(
    'FluxBeam',
    'https://api.fluxbeam.xyz/v1/pools',
    (data) => ({
      pools: Array.isArray(data) ? data : (data.pools || []),
      sample: data[0]?.name || 'N/A',
    })
  ...
  console.log('┌──────────────────┬────────┬────────────┬─────────┬────────────────────┐');
  console.log('│ DEX              │ Status │ Pools      │ Latency │ Sample Pool        │');
  console.log('├──────────────────┼────────┼────────────┼─────────┼────────────────────┤');
  ...
  for (const r of results) {
    ...
    console.log(`│ ${name} │ ${status}     │ ${pools}      │ ${latency} │ ${sample} │`);
  }
  ...
  require('fs').writeFileSync('./dex-api-report.json', JSON.stringify(report, null, 2));
