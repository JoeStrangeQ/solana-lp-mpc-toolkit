...
async function main() {
  ...
  console.log(`Cluster Account (offset ${CLUSTER_OFFSET}): ${clusterAccAddress.toBase58()}`);
  ...
  if (!clusterAccountInfo) {
    console.error("❌ Cluster account not found! The cluster may not be initialized.");
  ...
  console.log(`✅ Cluster account found (${clusterAccountInfo.data.length} bytes)`);
  ...
  try {
    ...
    if (data.length > UTILITY_PUBKEYS_OFFSET + X25519_KEY_LENGTH) {
      const potentialKey = data.slice(UTILITY_PUBKEYS_OFFSET, UTILITY_PUBKEYS_OFFSET + X25519_KEY_LENGTH);
      ...
      if (isZero) {
        ...
        console.log(`const MXE_PUBLIC_KEY = new Uint8Array([${Array.from(potentialKey).join(', ')}]);`);
    ...
    if (arciumProgram.account && arciumProgram.account.cluster) {
      const clusterData = await arciumProgram.account.cluster.fetch(clusterAccAddress);
