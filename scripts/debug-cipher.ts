...
import { ARCIUM_DEVNET_CONFIG, generatePrivacyKeys, deriveSharedSecret } from '../src/lp-toolkit/services/arciumPrivacy';
...
async function main() {
  ...
  const sharedSecret = deriveSharedSecret(keys.privateKey, ARCIUM_DEVNET_CONFIG.mxePublicKey);
  ...
  console.log('Match:', plaintext.every((v, i) => v === decrypted[i]));
  ...
  console.log('Deserialized same as original:', 
  ...
  try {
    ...
    console.log('Match:', plaintext.every((v, i) => v === decrypted2[i]));
  } catch (e) {
