...
console.log(`Collateral: $${COLLATERAL_SOL_VALUE} SOL (${SOL_AMOUNT.toFixed(4)} SOL) + $${COLLATERAL_USDC_VALUE} USDC`);
...
async function main() {
  ...
  if (!privateKeyBase58) {
    console.error('Usage: node test_double_sided.cjs <base58_private_key> [--execute]');
    process.exit(1);
  }
  ...
  console.log(`   SOL: ${(solBalance / 1e9).toFixed(4)} (need ${SOL_AMOUNT.toFixed(4)} + gas)`);
  
  // Get USDC balance
  const usdcAta = PublicKey.findProgramAddressSync(
    [wallet.publicKey.toBuffer(), new PublicKey('TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA').toBuffer(), new PublicKey(USDC_MINT).toBuffer()],
  ...
  const quoteResponse = await fetch(`${LOOPSCALE_BASE_URL}/markets/quote/max`, {
    ...
    body: JSON.stringify({
      ...
      collateralFilter: [{ amount: 0, assetData: { Spl: { mint: SOL_USDC_POOL } } }],
      priceOverride: 1,
    }),
  });
  ...
  console.log('   APY:', (quote.apy / 10000) + '%', '| LTV:', (quote.ltv / 10000) + '%');
  ...
  console.log(`   SOL deposit: ${(SOL_RAW / 1e9).toFixed(4)} SOL (${SOL_RAW} lamports)`);
  console.log(`   USDC deposit: ${(totalUsdcRaw / 1e6).toFixed(2)} USDC (${USDC_RAW} collateral + ${BORROW_RAW} borrowed)`);
  ...
  const flashBorrowResponse = await fetch(`${LOOPSCALE_BASE_URL}/markets/creditbook/flash_borrow`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'user-wallet': walletAddress },
  ...
  if (!flashBorrowResponse.ok) {
    ...
    if (errorText.includes('additionalDeposits') || errorText.includes('unknown field')) {
      console.log('\n🔄 Retrying without additionalDeposits...');
      delete flashBorrowRequest.additionalDeposits;
      
      const retryResponse = await fetch(`${LOOPSCALE_BASE_URL}/markets/creditbook/flash_borrow`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'user-wallet': walletAddress },
        body: JSON.stringify(flashBorrowRequest),
      });
