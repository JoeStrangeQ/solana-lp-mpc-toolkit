...
async function runTests() {
  ...
  try {
    const result = calculateLeverageAmounts(100, 2, { baseAssetPrice: 100, quoteAssetPrice: 1 });
    if (result.totalPositionSize === 200 && result.borrowRequired === 100) {
      console.log('  ✅ Passed');
      passed++;
    } else {
      throw new Error(`Expected 200/100, got ${result.totalPositionSize}/${result.borrowRequired}`);
    }
  } catch (e) {
  ...
  try {
    ...
    if (healthy.status === 'healthy' && (danger.status === 'danger' || danger.status === 'warning')) {
      console.log('  ✅ Passed');
      passed++;
    } else {
      throw new Error(`Unexpected statuses: ${healthy.status}, ${danger.status}`);
    }
  } catch (e) {
    console.log(`  ❌ Failed: ${e}`);
    failed++;
