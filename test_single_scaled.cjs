/**
 * Test Loopscale Single-Sided USDC Position - Scaled Up
 * Based on working v3 script
 */
...
// Position parameters - SCALED UP
const COLLATERAL_USDC = 10;  // $10 USDC collateral
const LEVERAGE = 2.5;        // 2.5x leverage
const BORROW_AMOUNT_RAW = Math.floor((COLLATERAL_USDC * (LEVERAGE - 1)) * 1_000_000);

console.log('=== Single-Sided USDC Position (Scaled) ===');
console.log(`Collateral: $${COLLATERAL_USDC} USDC`);
console.log(`Borrow: $${(BORROW_AMOUNT_RAW / 1e6).toFixed(2)} USDC`);
console.log(`Total Position: $${COLLATERAL_USDC + BORROW_AMOUNT_RAW / 1e6}`);
...
async function main() {
  ...
  if (!privateKeyBase58) {
    console.error('Usage: node test_single_scaled.cjs <base58_private_key> [--execute]');
    process.exit(1);
  }
    ...
    depositCollateral: [{
      ...
      loanCreationParams: {
        MeteoraDlmmPositionCreate: {
          ...
          width: width,
          liquidityParameter: {
            amountX: 0,  // Single-sided USDC only
            amountY: totalRaw,
