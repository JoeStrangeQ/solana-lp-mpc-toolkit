...
export function deriveLoanPda({ userAddress, loanNonce }: { userAddress: Address; loanNonce?: number }) {
  const finalLoanNonce = loanNonce ?? Math.floor(100000 + Math.random() * 900000);
  ...
  const [loanPda] = PublicKey.findProgramAddressSync([walletPubkey.toBuffer(), u64LoanNonce], loopscaleProgramId);
