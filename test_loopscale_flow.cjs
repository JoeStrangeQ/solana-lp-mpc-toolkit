...
const BORROW_AMOUNT_RAW = Math.floor((COLLATERAL_USDC * (LEVERAGE - 1)) * 1_000_000); // In USDC decimals (6)
...
async function main() {
  ...
  console.log(`   Borrow Amount: ${BORROW_AMOUNT_RAW / 1_000_000} USDC (${BORROW_AMOUNT_RAW} raw)`);
  ...
  for (let i = 0; i < flashBorrowData.transactions.length; i++) {
    ...
    // Simulate
    console.log(`   Simulating tx ${i + 1}/${flashBorrowData.transactions.length}...`);
    ...
    if (simResult.value.err) {
      console.error(`   ❌ Simulation failed for tx ${i + 1}:`, simResult.value.err);
