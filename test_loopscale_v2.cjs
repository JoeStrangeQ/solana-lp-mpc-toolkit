...
async function main() {
  ...
  if (!privateKeyBase58) {
    console.error('Usage: node test_loopscale_v2.cjs <base58_private_key> [--execute]');
    process.exit(1);
  }
  ...
  if (process.argv.includes('--execute')) {
    ...
    freshVtx.signatures = Array(msg.header.numRequiredSignatures).fill(Buffer.alloc(64));
    for (const s of tx.signatures) {
      const pk = new PublicKey(s.publicKey);
      const idx = msg.staticAccountKeys.findIndex(k => k.equals(pk));
      if (idx !== -1) freshVtx.signatures[idx] = Buffer.from(s.signature, 'base64');
    }
