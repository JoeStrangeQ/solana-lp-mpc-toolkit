...
async function main() {
  ...
  console.log(
    `   ✅ Private key: ${Buffer.from(privateKey).toString("hex").slice(0, 16)}...`,
  );
  console.log(
    `   ✅ Public key: ${Buffer.from(publicKey).toString("hex").slice(0, 16)}...`,
  );
  ...
  console.log(
    `   ✅ Client derives: ${Buffer.from(clientSecret).toString("hex").slice(0, 16)}...`,
  );
  console.log(
    `   ✅ MXE derives: ${Buffer.from(mxeSecret).toString("hex").slice(0, 16)}...`,
  );
  ...
  console.log(
    `   ✅ First element: ${ciphertext[0][0].toString().slice(0, 20)}...`,
  );
  ...
  console.log(
    `   ✅ All values match original: ${allMatch ? "YES ✓" : "NO ✗"}`,
  );
  ...
  try {
    ...
    const wrongMatch = strategyParams.every(
      (val, i) => val === wrongDecrypt[i],
    );
    if (wrongMatch) {
      console.error(
        "❌ CRITICAL: Wrong key decrypted correctly - security broken!",
      );
      process.exit(1);
    } else {
      console.log(
        `   ✅ Wrong key produces garbage: ${wrongDecrypt[0].toString().slice(0, 10)}...`,
      );
      console.log(`   ✅ Privacy verified: only correct key can decrypt`);
    }
  } catch (e) {
    console.log(
      `   ✅ Wrong key throws error (expected): ${e.message?.slice(0, 30) || "decryption failed"}`,
    );
    console.log(`   ✅ Privacy verified: only correct key can decrypt`);
  }
  ...
  console.log(
    `   Original: $${Number(posValue) / 1e6} value, $${Number(posFees) / 1e6} fees`,
  );
  console.log(
    `   Decrypted: $${Number(decValue) / 1e6} value, $${Number(decFees) / 1e6} fees`,
  );
