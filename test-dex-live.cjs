...
async function main() {
  ...
  if (await testDEX(
    ...
    (data) => {
      if (Array.isArray(data) && data.length > 0) {
        const top = data.find(p => p.name?.includes('SOL') && p.name?.includes('USDC'));
        return { success: true, pools: data.length, topPool: top?.name || data[0]?.name || 'Unknown' };
      }
  ...
  if (await testDEX(
    ...
    (data) => {
      if (Array.isArray(data) && data.length > 0) {
        return { success: true, pools: data.length, topPool: data[0]?.pool_address?.slice(0, 8) || 'Unknown' };
      }
      if (data.pools && Array.isArray(data.pools)) {
        return { success: true, pools: data.pools.length, topPool: 'via pools array' };
      }
  ...
  if (await testDEX(
    ...
    (data) => {
      if (data.whirlpools && Array.isArray(data.whirlpools)) {
        const top = data.whirlpools.find(p => p.tokenA?.symbol === 'SOL' && p.tokenB?.symbol === 'USDC');
        return { success: true, pools: data.whirlpools.length, topPool: top ? 'SOL-USDC' : data.whirlpools[0]?.address?.slice(0, 8) };
      }
  ...
  if (await testDEX(
    ...
    (data) => {
      if (data.success && data.data?.data) {
        return { success: true, pools: data.data.data.length, topPool: data.data.data[0]?.id?.slice(0, 8) || 'Unknown' };
      }
  ...
  if (await testDEX(
    ...
    (data) => {
      if (Array.isArray(data) && data.length > 0) {
        return { success: true, pools: data.length, topPool: data[0]?.name || 'Unknown' };
      }
      if (data.pools) {
        return { success: true, pools: Object.keys(data.pools).length, topPool: 'via pools object' };
      }
  ...
  if (await testDEX(
    ...
    (data) => {
      if (data.pools && Array.isArray(data.pools)) {
        const top = data.pools.find(p => p.name?.includes('USDC') && p.name?.includes('USDT'));
        return { success: true, pools: data.pools.length, topPool: top?.name || data.pools[0]?.name || 'Unknown' };
      }
  ...
  if (await testDEX(
    ...
    (data) => {
      if (Array.isArray(data) && data.length > 0) {
        return { success: true, pools: data.length, topPool: `${data[0]?.symbolX}-${data[0]?.symbolY}` || 'Unknown' };
      }
      return { success: false, error: 'No pools returned' };
    }
  )) passed++;
