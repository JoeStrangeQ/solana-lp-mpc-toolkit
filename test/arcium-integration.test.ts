...
async function main() {
  ...
  console.log(`   ✅ Private key: ${Buffer.from(keys.privateKey).toString('hex').slice(0, 16)}...`);
  console.log(`   ✅ Public key: ${Buffer.from(keys.publicKey).toString('hex').slice(0, 16)}...`);
  ...
  const mxeSharedSecret = x25519.getSharedSecret(mockMxePrivate, keys.publicKey);
  
  // Verify both sides derive same secret
  const secretsMatch = Buffer.from(clientSharedSecret).equals(Buffer.from(mxeSharedSecret));
  console.log(`   ✅ Client shared secret: ${Buffer.from(clientSharedSecret).toString('hex').slice(0, 16)}...`);
  console.log(`   ✅ MXE shared secret: ${Buffer.from(mxeSharedSecret).toString('hex').slice(0, 16)}...`);
  ...
  console.log(`   Plaintext: [${plaintext.map(n => n.toString()).join(', ')}]`);
  
  const ciphertext = cipher.encrypt(plaintext, nonce);
  console.log(`   ✅ Ciphertext: [${ciphertext.map(arr => arr[0].toString().slice(0, 10) + '...').join(', ')}]`);
  
  // Test 4: Decryption
  console.log('\n4️⃣ Testing Decryption...');
  const decrypted = cipher.decrypt(ciphertext, nonce);
  console.log(`   Decrypted: [${decrypted.map(n => n.toString()).join(', ')}]`);
  ...
  console.log(`   ✅ Ciphertext length: ${encrypted.ciphertext.length} elements`);
  console.log(`   ✅ Public key: ${encrypted.publicKey.slice(0, 20)}...`);
  
  // Test decryption
  const decryptedIntent = privacyService.decryptStrategy(encrypted);
  console.log(`   ✅ Decrypted totalValueUSD: ${decryptedIntent?.totalValueUSD}`);
  ...
  const encryptedPos = privacyService.encryptPositionValue(positionValue, positionFees);
