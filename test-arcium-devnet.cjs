...
async function main() {
  ...
  console.log(
    "   Program ID:",
    arciumEnv.arciumProgramId?.toString() || "Not set",
  );
  ...
  try {
    ...
    if (mxePublicKey) {
      console.log(
        "✅ MXE Public Key:",
        Buffer.from(mxePublicKey).toString("hex").slice(0, 32) + "...",
      );
      ...
      console.log(
        "⚠️  MXE public key not available (may need to deploy MXE first)",
      );
    ...
    console.log(
      "   For hackathon: we can use mock encryption or deploy our own MXE.",
    );
  }
